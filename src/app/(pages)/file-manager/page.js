...
const FileManager = () => {
  ...
  const fetchItems = useCallback(
    (path) => {
      cms.fetchItems(bucketName, path, setItems, setLoading);
    },
    [bucketName],
  );
  ...
  const handleEditClick = async (item) => {
    try {
      const res = await axios.get(
        `https://us-central1-onboarding-bot-14200.cloudfunctions.net/expressApi/cms/get-content?bucketName=${bucketName}&filePath=${item.name}&cloud=${cloud}`,
      );
  ...
  const handleEditSubmit = () => {
    cms.editContent(bucketName, editPath, editContent, setEditModal); // Call the service method
  };
  ...
  const handleDownloadFile = (item) => {
    cms.downloadFile(bucketName, item.name); // Call the service method
  };
      ...
      {domains.length > 0 ? (
        <div className="container py-5" style={{ minHeight: "100vh" }}>
          ...
          <Modal
            ...
            centered
            size="xl" // Increase the modal size
            ...
            <Modal.Body>
              <Form>
                <Form.Group controlId="fileContent">
                  ...
                  <Form.Control
                    as="textarea"
                    rows={15} // Increase the rows to display more content without scrollbar
                    value={editContent}
                    onChange={(e) => setEditContent(e.target.value)}
                    style={{ fontFamily: "monospace", fontSize: "1rem" }} // Monospace for a more professional look
                  />
                </Form.Group>
              </Form>
            </Modal.Body>
